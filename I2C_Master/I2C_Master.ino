...
uint8_t slave2 = 0x04 // 0b 00000100
...
void master_write_address(uint8_t slave_address) {
  ...
  TWCR &= ~(0 << TWSTA); // START 비트 0으로 만들어줌
  TWCR |= (1 << TWINT); // 통신 시작
  while (!(TWCR & (1 << TWINT))); // TWINT가 다시 1 될때까지 기다림
...
void master_read_data(uint8_t data) {
  TWCR |= (1 << TWINT); // 통신 시작
  while (!(TWCR & (1 << TWINT))); // TWINT가 다시 1 될때까지 기다림
  while ((TWSR & 0xF8) != 0x50); // TWSR에 0x50 입력될 때까지 기다림 // 안되면 0x58
  data = TWDR; // data에 TWDR값 입력
}
